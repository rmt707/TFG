...
<html>
    ...
    <body>    
        ...
        <div id="registro" style="overflow-x:auto; padding-left: 10px; padding-right: 10px;">
            <table>
                <h5>Información de Humedad (%) y Temperatura (ºC) por Hora</h5>
                <tr>
                ...
                <?php 
                    //Muestra el registro de las últimas 24 horas
                    $query = "SELECT humedad.anyo, humedad.mes, humedad.dia, humedad.hora, Max(humedad.humedad) AS MaxHumedad, round(Avg(humedad.humedad),2) AS PromedioHumedad, Min(humedad.humedad) AS MinHumedad, Max(temperatura.temperatura) AS MaxTemperatura, round(Avg(temperatura.temperatura),2) AS PromedioTemperatura, Min(temperatura.temperatura) AS MinTemperatura FROM humedad INNER JOIN temperatura ON (humedad.dia = temperatura.dia) AND (humedad.mes = temperatura.mes) AND (humedad.anyo = temperatura.anyo) AND (humedad.hora = temperatura.hora) AND (humedad.minutos = temperatura.minutos) AND (humedad.segundos = temperatura.segundos) GROUP BY humedad.anyo, humedad.mes, humedad.dia ORDER BY humedad.anyo DESC, humedad.mes DESC, humedad.dia DESC, humedad.hora DESC";
                    $result = mysqli_query($bbdd, $query);

                    $i = 0;
                    while(++$i < 23 && $row = mysqli_fetch_array($result, MYSQLI_NUM))
                    {      
                        print "<tr><td>{$row[0]}</td><td>{$row[1]}</td><td>{$row[2]}</td><td>{$row[3]}</td><td>{$row[4]}</td><td>{$row[5]}</td><td>{$row[6]}</td><td>{$row[7]}</td><td>{$row[8]}</td><td>{$row[9]}</td></tr>";
                    }  
                ?>
            </table>
        </div> 
